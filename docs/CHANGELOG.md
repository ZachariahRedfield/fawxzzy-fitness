...
- Reduced BottomActionBar dock corner radius from `rounded-2xl` to `rounded-xl` for a tighter system-style silhouette.
...
- Makes the dock read less like a card and more like native app chrome while preserving existing behavior and spacing contracts.
